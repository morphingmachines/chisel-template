...
object gcd extends ^.playground.build.CommonModule with SbtModule with ScalafmtModule with ScalafixModule  with ScalacOptions { m => 
  ...
  object test extends ScalaTests with ScalaTest with ScalafmtModule with ScalafixModule{
    override def sources = T.sources {Seq(PathRef(os.pwd/ "src" / "test"))}
  }
}
